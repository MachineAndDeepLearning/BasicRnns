...
class SimpleRNN(object):
	...
	def fit(self, X, Y, learning_rate=10e-1, mu=0.99, reg=1.0, activation=T.tanh, epochs=100, show_fig=True):
		D = X[0].shape[1]
		K = len(set(Y.flatten()))
		N = len(Y)
		M = self.M
		...
		# define theano input output
		thX = T.fmatrix('X')
		thY = T.ivector('Y')

		# define recurrence for theano scan
